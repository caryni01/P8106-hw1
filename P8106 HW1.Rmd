...
# show number of predictors (excluding intercept)
num_coff_2 = coef(lasso_model_1$finalModel, lasso_model_1$bestTune$lambda) 
sum(num_coff_2 != 0) - 1
```

It can be seen that both lambda.min and 1se generate a model with 37 predictors.The selected lambda for lowest cv rmse is 62.89 while the 1se rule gives lambda of 372.1. Since the lasso model based on 1se rule has smaller test mse which is 4.21e8 then lambda.min which is 4.40e8, the lasso model based on 1se is favored. 
